...
class QuadTree {
   private:
      ...
      void computeForce(int i, Node* node, Planet* planets, double &fx, double &fy) const {
         ...
         double distSqr = dx * dx + dy * dy + 1e-4; // epsilon to avoid division by zero
   
         // If node is a leaf with a single planet and it is not planet i.
         if (node->children[0] == nullptr && node->planetIndex != -1) {
   ...
   public:
      ...
      ~QuadTree() {
         std::cout << "Deleting QuadTree" << std::endl;  
         delete root;
      }
   ...
   //------------------------------------------------------------------------------
   // Update function using the Barnes–Hut QuadTree
   //------------------------------------------------------------------------------
...
int main(int argc, const char** argv){
   ...
   gettimeofday(&start, NULL);
   for (int i = 0; i < timesteps; i++) {
      std::cout << "computing timestep " << i << std::endl;
      update_planets(planets, buffer);
      Planet* temp = planets;
      planets = buffer;
      buffer  = temp;
