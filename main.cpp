...
void insertBody(QuadNode* node, int i, struct Planet* bodies) {
    ...
    if (node->NW == NULL) {
        ...
        if (bodies[oldIndex].x <= node->xmid) {
            if (bodies[oldIndex].y >= node->ymid)
                insertBody(node->NW, oldIndex, bodies);
            else
                insertBody(node->SW, oldIndex, bodies);
        } else {
            if (bodies[oldIndex].y >= node->ymid)
                insertBody(node->NE, oldIndex, bodies);
            else
                insertBody(node->SE, oldIndex, bodies);
        }
    }
    if (bodies[i].x <= node->xmid) {
        if (bodies[i].y >= node->ymid)
            insertBody(node->NW, i, bodies);
        else
            insertBody(node->SW, i, bodies);
    } else {
        if (bodies[i].y >= node->ymid)
            insertBody(node->NE, i, bodies);
        else
            insertBody(node->SE, i, bodies);
    }
}

void computeForce(QuadNode* node, struct Planet* bodies, int i, double theta, double G, double softening, double *fx, double *fy) {
    if (node == NULL) return;
    ...
    if (node->NW == NULL) {
        if (node->bodyIndex == i || node->bodyIndex == -1) return;
    ...
    if (s / dist < theta) {
        ...
        *fy += F * dy / dist;
        ...
        computeForce(node->NW, bodies, i, theta, G, softening, fx, fy);
        computeForce(node->NE, bodies, i, theta, G, softening, fx, fy);
        computeForce(node->SW, bodies, i, theta, G, softening, fx, fy);
        computeForce(node->SE, bodies, i, theta, G, softening, fx, fy);
    }
}
...
int main(int argc, const char** argv) {
    ...
    for (int step = 0; step < timesteps; step++) {
        ...
        double maxSpan = fmax(maxX - minX, maxY - minY);
        double half_size = maxSpan / 2.0 + 1e-6;
        QuadNode* root = createNode(cx, cy, half_size);
        for (int i = 0; i < nplanets; i++) {
        ...
        for (int i = 0; i < nplanets; i++) {
            ...
            computeForce(root, planets, i, theta, G, softening, &fx, &fy);
        ...
        freeTree(root);
        if (step % 10000 == 0) {
            printf("Timestep %d, location %f %f\n", step / 10000, planets[nplanets - 1].x, planets[nplanets - 1].y);
        }
    }
